...
8. **Optimisation des performances, intervalles et compteurs Anki (Zéro Latence & Progression)** :
   ...
   - Les textes des boutons s'adaptent dynamiquement aux vrais intervalles appliqués : ils affichent le temps FSRS réel en jours (ex: 5 j) pour les boutons qui valident le mot immédiatement (comme "Bien" sur une révision au 1er passage, ou "Facile"), et le temps en minutes (< 6 min, < 10 min) uniquement s'il s'agit d'une étape d'apprentissage intermédiaire.
   ...
   - Distinction claire entre nouveaux mots et mots déjà appris (révisions) : si vous répondez correctement (Bien ou Difficile) à un mot déjà appris dès le premier passage, il est validé immédiatement et sort de la session (1 seul passage requis). Seuls les mots nouveaux ou ceux qui ont échoué nécessitent 2 passages (1m/6m -> 10m -> acquis).
   - **Calibration FSRS par langue** : Possibilité de calibrer les 17 poids FSRS séparément pour chaque langue (Anglais, Espagnol, Allemand) sur la page Profil via un algorithme d'optimisation (Coordinate Descent). Un bouton "**Par défaut**" permet à tout moment de restaurer les poids initiaux standard de l'algorithme.
   - **Mélange intelligent des erreurs et espacement (Spacing)** : 
     - Les cartes ayant échoué (1 min de délai) sont présentées en avance (jusqu'à 40 secondes d'avance) pour se mélanger de manière fluide avec les nouveaux mots en cours de session.
     - Un historique d'exclusion filtre les 2 à 3 dernières cartes répondues pour éviter que la même carte (en erreur ou en confirmation) ne réapparaisse immédiatement au tour suivant, garantissant un espacement cognitif optimal.
